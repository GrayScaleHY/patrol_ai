...
def patrol_match(input_data):
    ...
    for scale in pointers:
        ...
        out_pointers[scale] = (int(coor[0]), int(coor[1]))
    ...
    for label in bboxes:
        ...
        box_ = [int(max(0, r[0])), int(max(0, r[1])), int(min(W, r[2])), int(min(H, r[3]))]
    ...
    for label in out_bboxes:
        ...
        cv2.putText(img_tag_, label, (int(o_[0]), int(o_[1])),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), thickness=1)

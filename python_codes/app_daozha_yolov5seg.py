...
def rank_bbox(out_data_list, data_masks, roi, type='iou'):
...
def inspection_daozha_detection(input_data, debug=False):
    ...
    if roi is not None:  # 如果配置了感兴趣区域，则画出感兴趣区域
        ...
        if dim == 1:
            if len(roi) == 4:
                cv2.rectangle(img_ref_, (int(roi[0]), int(roi[1])), (int(roi[2]), int(roi[3])), (255, 0, 255),
                              thickness=1)
                cv2.putText(img_ref_, "roi", (int(roi[0]), int(roi[1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (255, 0, 255),
                            thickness=1)

        else:
            for i in range(len(roi)):
                cv2.rectangle(img_ref_, (int(roi[i][0]), int(roi[i][1])), (int(roi[i][2]), int(roi[i][3])),
                              (255, 0, 255),
                              thickness=1)
                cv2.putText(img_ref_, "roi" + str(i + 1), (int(roi[i][0]), int(roi[i][1]) - 5),
    ...
    if roi is not None:
        ...
        if M is None:
            ...
            if dim == 1:
                ...
                for i in range(len(roi)):
                    coors = [(roi[i][0], roi[i][1]), (roi[i][2], roi[i][1]), (roi[i][2], roi[i][3]),
                             (roi[i][0], roi[i][3])]
                    coors_ = []
                    for coor in coors:
                        coors_.append(list(convert_coor(coor, M)))
        ...
        if dim == 1:
            ...
            for i in range(len(c)):
                # print(c[i])
                cv2.rectangle(img_tag_, (int(c[i][0]), int(c[i][1])), (int(c[i][2]), int(c[i][3])), (255, 0, 255),
                              thickness=1)
                cv2.putText(img_tag_, "roi" + str(i), (int(c[i][0]), int(c[i][1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (255, 0, 255),
                            thickness=1)
